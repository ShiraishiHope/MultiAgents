...
public class InfectionStatsUI : MonoBehaviour
{
    ...
    private string FormatStatistics(InfectionStats stats)
    {
        ...
        if (showRecoveredCount)
            output += $"<color=cyan>● Recovered:</color> {stats.recovered} ({GetPercentage(stats.recovered, stats.total)})\n";
        
        if (showDeadCount)
            output += $"<color=grey>● Dead:</color> {stats.dead} ({GetPercentage(stats.dead, stats.total)})\n";
